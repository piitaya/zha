...
@MULTI_MATCH(
    cluster_handler_names={"legrand_cable_outlet_cluster"},
)
class LegrandWirePilotThermostat(PlatformEntity):
    ...
    _pilot_wire_mode: int | None = None
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize ZHA Thermostat instance."""
        super().__init__(unique_id, cluster_handlers, endpoint, device, **kwargs)
        self._cable_outlet_cluster = self.cluster_handlers.get(
            "legrand_cable_outlet_cluster"
        )
        self._cable_outlet_cluster.on_event(
            CLUSTER_HANDLER_ATTRIBUTE_UPDATED,
            self.handle_cluster_handler_attribute_updated,
        )
        self._supported_features = ClimateEntityFeature.PRESET_MODE
    ...
    def handle_cluster_handler_attribute_updated(
        ...
        """Handle attribute update from device."""
        if event.attribute_name == "pilot_wire_mode":
            self._pilot_wire_mode = event.attribute_value

    @property
    def hvac_mode(self) -> HVACMode | None:
        """Return HVAC operation mode."""

        pilot_wire_mode = self._pilot_wire_mode
        if pilot_wire_mode is None:
            return None
        if pilot_wire_mode == 5:
            return HVACMode.OFF
        if pilot_wire_mode >= 0:
            return HVACMode.HEAT
        return None
    ...
    def preset_mode(self):
        ...
        if pilot_wire_mode == 1:
            return self.PRESET_COMFORT_MINUS_1
        if pilot_wire_mode == 2:
            return self.PRESET_COMFORT_MINUS_2
        if pilot_wire_mode == 3:
            return Preset.ECO
        if pilot_wire_mode == 4:
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        pilot_wire_mode = 5 if hvac_mode == HVACMode.OFF else 0
        mfg_code = self._device.manufacturer_code
        await self._cable_outlet_cluster.write_attributes_safe(
            {"pilot_wire_mode": pilot_wire_mode}, manufacturer=mfg_code
        )
